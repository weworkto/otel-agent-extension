...
Grab the latest JAR from [Releases](https://github.com/weworkto/otel-agent-extension/releases/latest):
...
RUN curl -fsSL -o /agent/opentelemetry-javaagent-extensions.jar \
    "https://github.com/weworkto/otel-agent-extension/releases/download/v${OTEL_EXT_VERSION}/opentelemetry-javaagent-extensions-${OTEL_EXT_VERSION}.jar"
```
...
```bash
git clone https://github.com/weworkto/otel-agent-extension.git
cd otel-agent-extension
./gradlew build
```
...
│   └── test/java/.../TraceIdResponseCustomizerTest.java
...
1. Update `otelAgentVersion` and `otelSdkVersion` in `gradle.properties`
